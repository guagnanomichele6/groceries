...
def consume_inventory_item(cursor, item_name, qty_needed, recipe_unit):
    ...
    if item:
        ...
        if new_qty <= 0:
            ...
            # Inserisce automaticamente nella lista della spesa
...
with tab_finanza:
    ...
    # Categorie con Investimenti e Giroconto
    expense_categories = [
    ...
    with sub_f2:
        ...
        if not acc_global_dict:
            ...
            with st.form("trans_form"):
                ...
                # PULSANTE RIPRISTINATO CORRETTAMENTE
    ...
    with sub_f6:
        ...
        if not df_summary.empty:
            ...
            # Escludiamo dal report delle spese vive i giroconti interni
            ...
            if st.button("💾 Salva Modifiche Dispensa"):
                ...
                for _, row in edited_inv.iterrows():
                    if pd.notna(row['Prodotto']) and str(row['Prodotto']).strip() != "":
                        ...
                        cursor.execute("INSERT INTO inventory (item_name, category, quantity, unit) VALUES (?, ?, ?, ?)",
                                       (str(row['Prodotto']).strip().capitalize(), cat_val, float(row['Quantità']), str(row['Unità'])))
                conn.commit()
                conn.close()
    ...
    with sub_p2:
        ...
        if not df_accs.empty and not df_shop.empty:
            ...
            with st.form("checkout_form"):
                ...
                chk_acc_id = acc_global_dict[chk_sel_acc]['id'] if 'acc_global_dict' in locals() and chk_sel_acc in acc_global_dict else chk_acc_dict[chk_sel_acc]['id']
...
with tab_impostazioni:
    ...
    st.caption("Crea una copia di sicurezza del database o ripristino uno stato precedente.")
